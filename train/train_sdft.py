...
Privileged context (`--pi-mode`), built from DeepMath columns:
...
# The student never sees it. Framing is baked in here (kept out of the template)
# so each --pi-mode is fully self-describing; the paper frames c as a worked
# example and asks the model to write its own response.
# ---------------------------------------------------------------------------
...
def build_sdft_dataset(
    ...
    `load_deepmath` yields (question, final_answer, r1_solution_*).
    """
...
def main():
    ...
    # privileged context
    p.add_argument("--pi-mode", default="full", choices=["full", "answer"],
                   help="Teacher-only privileged context: 'full' worked demo (paper "
                        "default) or 'answer' hint. See PI_* templates.")
    # SDFT objective
