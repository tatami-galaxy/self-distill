...
first experiment supports two matched arms:

  * ``--pi-mode none``   -- the critic reads exactly the policy prompt (the train_ppo.py control)
  * ``--pi-mode answer`` -- the critic also sees the gold final answer, using the same PI wording
                            and last-user-turn injection convention as SDFT
...
def build_ppo_pi_dataset(
    dataset: str = "deepmath", max_samples: int | None = None, pi_mode: str = "answer"
):
    ...
    def _map(row):
        ...
        privileged_context = (
            PI_ANSWER.format(answer=answer) if pi_mode == "answer" else ""
        )
        return {
...
class PPOPITrainer(PPOTrainer):
    ...
    def _build_value_prompts(self, rows, device):
        """Render answer-PI conversations with the base class's aligned tokenizer path."""
        conversations = []
        for i, row in enumerate(rows):
            if "privileged_context" not in row:
                raise RuntimeError(f"row {i} has no 'privileged_context' for pi_mode='answer'")
            if not str(row["privileged_context"]).strip():
                raise RuntimeError(f"row {i} has empty privileged information for pi_mode='answer'")
...
def main():
    ...
    p.add_argument("--pi-mode", default="answer", choices=PI_MODES,
                   help="PI exposed only to the critic. 'none' is the matched PPO control.")
    ...
    train_dataset = build_ppo_pi_dataset(
        args.dataset, max_samples=args.max_samples, pi_mode=args.pi_mode
    )
