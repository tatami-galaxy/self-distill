...
For each sampled problem:
  ...
  2. Sample one student rollout (temperature > 0).
  3. Re-score the rollout's own tokens with the student at T=1 via prompt_logprobs.
  4. Grade the boxed answer against gold (eval.utils.grade_answer).

Saves one JSONL record per rollout (correct and incorrect) under
data/credit_assignment/. The rollouts file is teacher-agnostic: it is generated
once and reused by every scoring pass (score_teacher.py, future OPSD/divergence).

Run from the repo root, e.g.:
    python -m credit_assignment.generate_rollouts \
        --student Qwen/Qwen3-1.7B --num-samples 8 --max-tokens 16384
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--score-batch-size", type=int, default=16,
                   help="Sequences per prompt_logprobs scoring batch.")
...
def main() -> None:
    ...
    # Collect completions, then self-score in batches.
    completions: list[list[int]] = [list(o.outputs[0].token_ids) for o in outputs]
    completion_texts: list[str] = [o.outputs[0].text for o in outputs]
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        for batch_start in range(0, len(problems), args.score_batch_size):
            batch = range(batch_start, min(batch_start + args.score_batch_size, len(problems)))
            seqs: list[tuple[list[int], int]] = []
            valid_idx: list[int] = []
            for i in batch:
                if not completions[i]:  # empty generation, nothing to score
                    continue
                full_ids = prompt_ids_list[i] + completions[i]
                seqs.append((full_ids, len(prompt_ids_list[i])))
                valid_idx.append(i)
            if not seqs:
                continue
            ...
            for i, per_token in zip(valid_idx, scored):
                prob = problems[i]
                comp_ids = completions[i]
                ...
                pred_answer = extract_boxed_answer(completion_texts[i])
